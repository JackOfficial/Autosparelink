        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="example1" class="table table-striped projects">
                    ...
                    <tbody>
                        @forelse($engineTypes as $engineType)
                        <tr>
                            <td>{{ $engineType->id }}</td>
                            ...
                            <td>
                                @if($engineType->icon_url)
                                    <img src="{{ asset('storage/' . $engineType->icon_url) }}"
                                         class="img-thumbnail"
                            ...
                            <td>
                                ...
                                <br>
                                <small class="text-muted">{{ $engineType->created_at->format('Y-m-d') }}</small>
                            </td>

                            <!-- Description -->
                            <td>{!! Str::limit($engineType->description, 50) !!}</td>

                            <!-- Actions -->
                            <td class="d-flex">
                                <a href="{{ route('admin.engine-types.edit', $engineType->id) }}"
                                   class="btn btn-info btn-sm mr-2">
                                    <i class="fas fa-edit"></i>
                                </a>

                                <form action="{{ route('admin.engine-types.destroy', $engineType->id) }}"
                                      method="POST"
