...
<section class="content" x-data="{ 
        ...
        <div class="col-md-6">
            <div class="input-group shadow-sm">
                ...
                <input type="text" 
                       x-model="search" 
                       class="form-control" 
                       placeholder="Search Model, Code, or Region..."
            ...
            <div class="box-header with-border" 
                ...
                <div class="row">
                    ...
                    <div class="col-xs-3 text-right">
                        <span class="spec-pill">{{ $specGroup->count() }} Variants</span>
                        ...
                        <table class="table table-hover" style="margin-bottom: 0;">
                            <thead>
                                <tr style="background: #fcfcfc; font-size: 11px; text-transform: uppercase; color: #777;">
                                    <th style="padding-left: 15px; width: 250px;">Identity & Market</th>
                            ...
                            <tbody>
                                @foreach($specGroup as $spec)
                                    @php
                                        $v = $spec->variant;
                                        $rowSearch = strtolower(($v->name ?? '') . ' ' . ($v->chassis_code ?? '') . ' ' . ($v->model_code ?? ''));
                                    @endphp
                                    <tr x-show="search === '' || '{{ $rowSearch }}'.includes(search.toLowerCase()) || '{{ $groupSearchTag }}'.includes(search.toLowerCase())">
                                        ...
                                        <td style="padding-left: 15px;">
                                            ...
                                            <div style="margin-top: 4px;">
                                                <span class="badge-code" title="Chassis Code">{{ $v->chassis_code ?? '---' }}</span>
                                                <span class="badge-code" title="Model Code">{{ $v->model_code ?? '---' }}</span>
                                            </div>
                                            <div style="font-size: 11px; margin-top: 5px; color: #666;">
                                                <i class="fa fa-globe text-muted"></i> 
                                                {{ $v->destinations->pluck('region_name')->join(', ') ?: 'Global Market' }}
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div style="display: flex; gap: 15px;">
                                                <div>
                                                    <small class="text-muted block text-uppercase" style="font-size: 9px;">Power Unit</small>
                                                    <div style="font-size: 12px;">
                                                        <strong>{{ $spec->engineType->name ?? 'N/A' }}</strong> 
                                                        <span class="text-muted">({{ $spec->transmissionType->name ?? 'N/A' }})</span>
                                                    </div>
                                                    <div style="font-size: 11px; color: #555;">
                                                        <i class="fa fa-bolt text-warning"></i> {{ $spec->horsepower }} HP / {{ $spec->torque }} Nm
                                                    </div>
                                                </div>
                                                <div style="border-left: 1px solid #eee; padding-left: 15px;">
                                                    <small class="text-muted block text-uppercase" style="font-size: 9px;">Drive & Steering</small>
                                                    <div style="font-size: 11px;">
                                                        <span class="label label-info">{{ $spec->driveType->name ?? 'N/A' }}</span>
                                                        <span class="label label-default">{{ $spec->steering_position }} Drive</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div style="font-size: 11px; line-height: 1.6;">
                                                ...
                                                <div><i class="fa fa-tint text-muted" style="width: 14px;"></i> {{ $spec->fuel_capacity }}L | {{ $spec->fuel_efficiency }}</div>
                                        ...
                                        <td class="text-center">
                                            @if($v->is_default)
                                                <span class="label label-warning" style="display:block; margin-bottom: 5px;">DEFAULT</span>
                                            @endif
                                        ...
                                        <td class="text-right" style="padding-right: 15px;">
                                            <div class="btn-group">
                                                <a href="{{ route('admin.specifications.edit', $spec->id) }}" class="btn btn-default btn-sm shadow-xs" title="Edit">
                                                    <i class="fa fa-pencil text-blue"></i>
                                                </a>
                                                <button class="btn btn-default btn-sm shadow-xs" title="Delete">
    ...
    <div x-show="search !== '' && !document.querySelectorAll('.spec-group-wrapper[style*=\'display: block\']').length" 
        ...
        <p>No results found for "<strong><span x-text="search"></span></strong>"</p>
    </div>
</section>
...
<style>
    ...
    .badge-code { background: #f0f0f0; border: 1px solid #ddd; padding: 1px 5px; border-radius: 3px; font-family: monospace; font-size: 10px; color: #555; margin-right: 3px; }
