@extends('admin.layouts.app')
@section('title', 'Categories')

@section('content')

{{-- 1. Initialize Alpine.js for real-time filtering --}}
<div x-data="{ 
    search: '',
    isParentVisible(parentName, childrenJson) {
        if (!this.search) return true;
        const s = this.search.toLowerCase();
        if (parentName.toLowerCase().includes(s)) return true;
        
        const children = JSON.parse(childrenJson);
        return children.some(child => child.category_name.toLowerCase().includes(s));
    },
    isChildVisible(childName) {
        if (!this.search) return true;
        return childName.toLowerCase().includes(this.search.toLowerCase());
    }
}">

{{-- ================= HEADER ================= --}}
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>
                    Categories
                    <small class="text-muted" x-show="!search">({{ $categories->count() }})</small>
                    <small class="text-primary" x-show="search" x-cloak>Filtering...</small>
                </h1>
            </div>
            <div class="col-sm-6 text-right">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="/admin">Home</a></li>
                    <li class="breadcrumb-item active">Categories</li>
                </ol>
            </div>
        </div>
    </div>
</section>

{{-- ================= CONTENT ================= --}}
<section class="content">
    {{-- Narrower Container for better readability --}}
    <div class="container-fluid" style="max-width: 900px; margin-left: 0;">

        {{-- FLASH MESSAGE --}}
        @if(session('success'))
            <div class="alert alert-success alert-dismissible fade show shadow-sm mb-3">
                <i class="fas fa-check-circle"></i> {{ session('success') }}
                <button type="button" class="close" data-dismiss="alert">&times;</button>
            </div>
        @endif

        {{-- ACTION & SEARCH BAR --}}
        <div class="row mb-3 align-items-center">
            <div class="col-md-8">
                <div class="input-group input-group-sm shadow-sm">
                    <div class="input-group-prepend">
                        <span class="input-group-text bg-white border-right-0">
                            <i class="fas fa-search text-muted"></i>
                        </span>
                    </div>
                    <input type="text" 
                           x-model="search" 
                           class="form-control border-left-0" 
                           placeholder="Filter categories or subcategories...">
                    <div class="input-group-append" x-show="search" x-cloak>
                        <button class="btn btn-outline-secondary border-left-0" @click="search = ''">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                </div>
            </div>
            <div class="col-md-4 text-right">
                <a href="{{ route('admin.categories.create') }}" class="btn btn-primary btn-sm shadow-sm">
                    <i class="fa fa-plus"></i> Add Category
                </a>
            </div>
        </div>

        {{-- CATEGORY GROUPS --}}
        @forelse($categories as $parent)
            <div class="card card-outline card-primary mb-3 shadow-sm" 
                 x-show="isParentVisible('{{ addslashes($parent->category_name) }}', '{{ addslashes($parent->children->toJson()) }}')"
                 x-transition:enter="transition ease-out duration-200"
                 x-transition:enter-start="opacity-0 transform scale-95"
                 x-transition:enter-end="opacity-100 transform scale-100">

                {{-- THINNER PARENT HEADER --}}
                <div class="card-header py-2 d-flex justify-content-between align-items-center bg-white">
                    <div class="d-flex align-items-center">
                        @if($parent->photo)
                            <img src="{{ asset('storage/' . $parent->photo) }}"
                                 class="img-thumbnail mr-2 elevation-1"
                                 style="width:40px; height:40px; object-fit:cover; border-radius: 6px;">
                        @else
                            <div class="mr-2 bg-light border d-flex align-items-center justify-content-center" style="width:40px; height:40px; border-radius: 6px;">
                                <i class="fas fa-image text-muted fa-xs"></i>
                            </div>
                        @endif

                        <div>
                            <span class="h6 mb-0 font-weight-bold text-dark">{{ $parent->category_name }}</span>
                            <small class="text-muted ml-2">({{ $parent->children->count() }})</small>
                        </div>
                    </div>

                    <div class="btn-group">
                        <a href="{{ route('admin.categories.edit', $parent->id) }}"
                           class="btn btn-link btn-xs text-info p-1" title="Edit">
                            <i class="fas fa-edit"></i>
                        </a>

                        <form action="{{ route('admin.categories.destroy', $parent->id) }}"
                              method="POST"
                              class="d-inline"
                              onsubmit="return confirm('Delete this parent and all its subcategories?');">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-link btn-xs text-danger p-1" title="Delete">
                                <i class="fas fa-trash"></i>
                            </button>
                        </form>
                    </div>
                </div>

                {{-- COMPACT CHILD TABLE --}}
                <div class="card-body p-0">
                    @if($parent->children->isNotEmpty())
                        <div class="table-responsive">
                            <table class="table table-sm table-hover mb-0" style="font-size: 0.875rem;">
                                <thead class="bg-light">
                                    <tr>
                                        <th class="pl-3" width="70">Photo</th>
                                        <th>Subcategory Name</th>
                                        <th class="text-center" width="100">Items</th>
                                        <th class="text-right pr-3" width="120">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($parent->children as $child)
                                        <tr x-show="isChildVisible('{{ addslashes($child->category_name) }}')"
                                            x-transition>
                                            <td class="pl-3">
                                                @if($child->photo)
                                                    <img src="{{ asset('storage/' . $child->photo) }}"
                                                         class="img-circle border shadow-sm"
                                                         style="width:30px; height:30px; object-fit:cover;">
                                                @else
                                                    <div class="img-circle bg-light border d-flex align-items-center justify-content-center" style="width:30px; height:30px;">
                                                        <i class="fas fa-folder fa-xs text-muted"></i>
                                                    </div>
                                                @endif
                                            </td>
                                            <td class="align-middle">
                                                <span class="font-weight-bold">{{ $child->category_name }}</span>
                                            </td>
                                            <td class="text-center align-middle">
                                                <span class="badge badge-light border text-muted">{{ $child->parts_count ?? 0 }}</span>
                                            </td>
                                            <td class="text-right pr-3 align-middle">
                                                <div class="btn-group">
                                                    <a href="{{ route('admin.categories.edit', $child->id) }}"
                                                       class="btn btn-xs btn-default text-primary border-0" title="Edit">
                                                        <i class="fas fa-pencil-alt"></i>
                                                    </a>
                                                    <button type="button" 
                                                            class="btn btn-xs btn-default text-danger border-0" 
                                                            onclick="if(confirm('Delete subcategory?')) document.getElementById('delete-{{ $child->id }}').submit();"
                                                            title="Delete">
                                                        <i class="fas fa-trash"></i>
                                                    </button>
                                                </div>
                                                <form id="delete-{{ $child->id }}" action="{{ route('admin.categories.destroy', $child->id) }}" method="POST" class="d-none">
                                                    @csrf @method('DELETE')
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                    @else
                        <div class="p-3 text-center text-muted small">
                            <i class="fas fa-info-circle mr-1"></i> No subcategories.
                        </div>
                    @endif
                </div>
            </div>
        @empty
            <div class="alert alert-light text-center py-4 border shadow-sm">
                <i class="fas fa-search mb-2 text-muted"></i>
                <p class="mb-0 text-muted">No categories found.</p>
            </div>
        @endforelse
    </div>
</section>
</div>

<style>
    [x-cloak] { display: none !important; }
    .table-hover tbody tr:hover { background-color: rgba(0,123,255,0.02); }
    .card-header .btn-link:hover { opacity: 0.7; text-decoration: none; }
</style>

@endsection