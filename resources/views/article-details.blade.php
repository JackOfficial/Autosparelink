...
<style>
    ...
    .share-btn { width: 40px; height: 40px; display: inline-flex; align-items: center; justify-content: center; transition: 0.3s; }
    .share-btn:hover { transform: translateY(-3px); box-shadow: 0 4px 8px rgba(0,0,0,0.15); }
    ...
    <div class="row px-xl-5">
        <div class="col-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-light shadow-sm rounded-pill px-4">
                    <li class="breadcrumb-item"><a class="text-primary" href="{{ url('/') }}">Home</a></li>
                    <li class="breadcrumb-item"><a class="text-primary" href="{{ route('blogs.index') }}">Insights</a></li>
                ...
                <div class="bg-white p-4 mb-4 shadow-sm rounded">
                    ...
                    @foreach($categories as $cat)
                        <a href="{{ route('blogs.index', ['category' => $cat->slug]) }}" 
                           class="d-flex justify-content-between align-items-center mb-2 text-dark text-decoration-none py-1 border-bottom-hover">
                            <span>{{ $cat->name }}</span>
                ...
                <div class="bg-white p-4 mb-4 shadow-sm rounded">
                    ...
                    @foreach($recentPosts as $recent)
                        <div class="d-flex align-items-center mb-3">
                            <img src="{{ $recent->newsPhoto ? asset('storage/' . $recent->newsPhoto->file_path) : asset('defaults/no-image.jpg') }}" 
            ...
            <article class="bg-white p-4 p-lg-5 mb-4 shadow-sm rounded">
                <header class="mb-4 text-center text-md-left">
                    ...
                    <div class="d-flex flex-wrap justify-content-center justify-content-md-start text-muted mt-3 py-3 border-top border-bottom">
                        ...
                        <div class="mr-4 mb-2"><i class="fa fa-eye text-primary mr-2"></i> {{ number_format($post->views ?? 0) }} Views</div>
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="card h-100 border-0 shadow-sm hover-grow">
                                ...
                                <div class="card-body p-3">
                                    <h6 class="mb-0"><a href="{{ route('blogs.show', $rel->slug) }}" class="text-dark stretched-link">{{ $rel->title }}</a></h6>
                                </div>
                            </div>
                        </div>
                    @endforeach
